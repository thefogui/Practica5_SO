...
p_block cercar_bloc_lliure(size_t size) {
	...
	best_fit = current;
	while(current != 0){ //mientras no sea el final
		current = current->next;
...
void free(void *ptr){
    ...
    current = (p_block)ptr -1;//cast del puntero void a puntero p_block
    current->free = 1; //pasa el indicador a 1
    aux = current->next;
    while(aux->free == 1){
        current->size = current->size + aux->size; //incrementa el size
        current->next = aux->next; //cambia el puntero a el seguiente
        aux = NULL; //elimina el puntero actual
        aux = current->next;
    }
}
